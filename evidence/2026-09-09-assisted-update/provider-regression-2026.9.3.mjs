...
const file=fs.readdirSync(dist).find(f=>/^loader-runtime-load-.*\.mjs$/.test(f));
...
const match=mod.y, publish=snap.E, metadata=snap.s, clear=snap.t;
...
if(process.argv[3]) test('prior cache mutation bug reproduced and corrected',()=>{
 ...
 const ctx=vm.createContext({getRuntimeConfigSnapshotMetadata:snap.s,resolveProviderConfig:mod.T,hashRuntimeConfigValue:snap.d});
