...
Result: merged the advanced ledger base without rewriting incident history; corrected PR #3; verified an encrypted S3 config rollback; disabled Jev in one config line and restarted once. A command cron completed, Telegram E2E delivery passed, and 72/72 RPCs passed over 18 minutes. Three transient event-loop degradations and a prolonged Scout agent cron keep engineering acceptance PARTIAL / DEGRADED. The sanitized inventory, GitHub precedents, limits, and rollback are in `handoffs/2026-09-18_jev-disable-observation-plugin-inventory.md`.

Scope boundary: no uninstall, dependency removal, Doctor fix, OpenClaw update, service rewrite, provider/auth/model change, or automatic repair of unrelated plugins. Keep/remove choices remain deferred pending sustained stability and Scout cron outcome.
